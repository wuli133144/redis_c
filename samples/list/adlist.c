...
{
    struct list *list; 
    if ((list = malloc(sizeof(*list))) == NULL)  //直接创建 
        return NULL;
...
void listRelease(list *list)
{
    ...
    while(len--) {
        next = current->next;
        if (list->free) list->free(current->value); //释放节点 
        free(current);  //释放节点值
        current = next;
    }
    free(list);
...
list *listAddNodeHead(list *list, void *value) //添加头node
{
    listNode *node;
    ...
    if ((node = malloc(sizeof(*node))) == NULL)
        return NULL;
    node->value = value;
    if (list->len == 0) { //如果开始没有节点 head =tail=node 
...
listNode *listSearchKey(list *list, void *key)
{
    ...
    while((node = listNextElement(iter)) != NULL) {
        if (list->match) {
            if (list->match(node->value, key)) { //找到匹配的节点
                listReleaseIterator(iter);
                return node;
            }
        } else {
...
listNode *listIndex(list *list, int index) {   //根据索引找到值
    listNode *n;
